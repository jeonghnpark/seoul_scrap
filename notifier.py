...
class Notifier:
    ...
    def format_message(self, reservation):
        ...
        url = reservation.get('url', '#') # URL이 없는 경우 임시 링크로 처리
        
        rcpt_bgndt = reservation.get('rcpt_bgndt', '') # 접수 시작 일시
        rcpt_enddt = reservation.get('rcpt_enddt', '') # 접수 종료 일시
...
if __name__ == "__main__":
    ...
    async def test():
        try:
            ...
            await notifier.send_message("테스트 메시지입니다. 이 메시지가 텔레그램으로 전송됩니다.")
